...
\begin{center}
  \begin{tabular}{l l l}
    ...
    \hline
    MicroK8s\cite{microk8s}        & v1.32.9           & Kubernetes distribution \\
    Docker \cite{docker}         & 20.10+            & Image building \\
...
The deployment is defined by a set of Kubernetes \cite{kubernetes} manifests in \texttt{k8s/base/hub/}:
...
\begin{itemize}
  ...
  \item The two machines are connected via a \textbf{ZeroTier} \cite{zerotier} virtual private network, providing a secure layer-2 tunnel without requiring public IP exposure on the MicroK8s node.
\end{itemize}

\paragraph{HTTP Traffic (Client $\rightarrow$ Hub).}
An Nginx \cite{nginx} reverse proxy on the public VPS forwards HTTPS traffic to the MicroK8s Ingress:
\begin{enumerate}
