...
From a CAP theorem perspective, it is \textbf{AP-oriented}: its primary goal is to guarantee \textbf{Availability} and \textbf{Partition Tolerance}, accepting eventual consistency across hub instances.
...
Hubs synchronize this state through a \textbf{gossip protocol} over UDP, using Proto Buf for message serialization.
The gossip layer handles two categories of events:
\begin{itemize}
    \item \textbf{Peer lifecycle}: join, leave, alive, suspicious, and dead notifications, enabling distributed peer discovery and failure detection without a central coordinator.
    \item \textbf{Room lifecycle}: room activation, player joins, game start, and room closure, ensuring all hubs share a consistent (eventually) view of available game sessions.
\end{itemize}

Messages are propagated using a \textbf{fanout-based forwarding} strategy: each hub forwards received gossip messages to a bounded subset of its known peers.
...
If no room is available, the hub triggers the activation of a new room instance and communicates to other peers that.
