...
A leader-based architecture would provide strong consistency but would sacrifice availability during leader elections or network partitions.
Since the Hub's primary role is matchmaking (returning a joinable room to clients), temporary inconsistencies between hubs are tolerable and self-correcting via gossip convergence or system evolving.
...
The StatefulSet guarantees stable hostnames.
Each hub pod exposes two network interfaces: HTTP API and UDP Gossip socket.

\paragraph{Room Instances.}
Each Room Server runs as an independently from HubServer.
Room Servers comes from a Game Port (TCP) where the players connect to the game and a HTTP port that is used from HubServers for health checks.

An overview of room is shown in \cref{fig:room-overview}

\begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{figures/room-overview.png}
    \caption{Room Server internal architecture.}
...
The Room lifecycle is shown in figure \cref{fig:design-hub-pov-room-lifecycle}
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{figures/hub-pov-room-lifecycle}
    \caption{Room lifecycle}
    \label{fig:design-hub-pov-room-lifecycle}
\end{figure}
...
\begin{itemize}
  ...
  It contains the grid, the list of players, and active bombs.
...
The hub consults its local \texttt{HubState} for a joinable room (status \texttt{ACTIVE} and \texttt{player\_count < max\_players}).
...
If activation fails, the hub returns HTTP 503. The interaction between client and hub is shown in figure \ref{fig:client-hub-interaction}

\begin{figure}
  \centering
  \includegraphics[width=0.7\textwidth]{figures/hub-http-high-level.png}
  \caption{Client interaction with the Hub cluster. Clients connect to a single entry point without knowledge of which hub instance serves their request.}
  \label{fig:client-hub-interaction}
\end{figure}
...
Full logic is shown in figure \cref{fig:gossip-pipeline}
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{figures/hub-server-gossip-interaction}
    \caption{Gossip message processing pipeline. The forwarder is marked alive regardless of whether the message is a duplicate, providing implicit heartbeating.}
    \label{fig:gossip-pipeline}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{Tick-Based Broadcast}: The Room Server does not respond to inputs immediately \cite{nystrom2014game}.
...
The listener thread dispatches each received datagram to a bounded thread pool (\texttt{ThreadPoolExecutor}) to avoid blocking the receive loop while bounding concurrency.
...
While active gameplay reads and writes to RAM, dumping the state to a \texttt{.pkl} file provides a persistent snapshot of the game. 
...
The failure detection system is shown in figure \cref{fig:design-failure-detector}

\begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{figures/failure-detector}
    \caption{Failure detector state machine.}
    \label{fig:design-failure-detector}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Malformed messages}: protobuf deserialization errors are caught in \texttt{HubSocketHandler} and logged, preventing invalid data from corrupting hub state.
...
\begin{itemize}
  \item \textbf{Server Failure and Recovery}: If a Room Server process crashes unexpectedly, the match state is not permanently lost. Upon restart, the server can deserialize the latest \texttt{.pkl} snapshot to restore the \texttt{Game Engine}. 
